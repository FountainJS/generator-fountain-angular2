...
describe('Reducers', () => {
  ...
  describe('visibility reducer', () => {
    it('should handle initial state', () => {
      expect(visibility(undefined, {}).type).toEqual(filters.SHOW_ALL);
      expect(visibility(undefined, {}).filter()).toEqual(true);
    });

    it('should handle SHOW_COMPLETED', () => {
      const showCompleted = visibility(() => true, {type: filters.SHOW_COMPLETED});
    ...
    it('should handle SHOW_ACTIVE', () => {
      const showActive = visibility(() => true, {type: filters.SHOW_ACTIVE});
